...
$badge_labels = array(
    'draft'     => __( 'Borrador', 'almc-verifactu' ),
    'queued'    => __( 'En cola', 'almc-verifactu' ),
    'submitted' => __( 'Enviada', 'almc-verifactu' ),
    'accepted'  => __( 'Aceptada', 'almc-verifactu' ),
    'rejected'  => __( 'Rechazada', 'almc-verifactu' ),
    'error'     => __( 'Error', 'almc-verifactu' ),
    'cancelled' => __( 'Anulada', 'almc-verifactu' ),
);
?>
...
<div class="almc-vf-metabox" data-order-id="<?php echo esc_attr( $order_id ); ?>">
    ...
    <?php if ( ! $is_configured ) : ?>
        <p class="almc-vf-notice-warning">
            ...
            printf(
                ...
                wp_kses_post( __( 'Plugin no configurado. %1$sConfigurar ajustes%2$s', 'almc-verifactu' ) ),
                '<a href="' . esc_url( admin_url( 'admin.php?page=almc-verifactu' ) ) . '">',
                '</a>'
            );
            ?>
        </p>
    ...
    <?php elseif ( ! empty( $vf_uuid ) ) : ?>
        ...
        <div class="almc-vf-status-row">
            <strong><?php esc_html_e( 'Estado:', 'almc-verifactu' ); ?></strong>
        ...
        <?php if ( ! empty( $vf_invoice_number ) ) : ?>
            <div class="almc-vf-info-row">
                <strong><?php esc_html_e( 'Factura:', 'almc-verifactu' ); ?></strong>
        ...
        <!-- UUID -->
        <div class="almc-vf-info-row">
            <strong><?php esc_html_e( 'UUID:', 'almc-verifactu' ); ?></strong>
            <code class="almc-vf-uuid" id="almc-vf-uuid-<?php echo esc_attr( $order_id ); ?>"><?php echo esc_html( $vf_uuid ); ?></code>
            <button type="button" class="button button-small almc-vf-copy-btn" data-target="almc-vf-uuid-<?php echo esc_attr( $order_id ); ?>" title="<?php esc_attr_e( 'Copiar UUID', 'almc-verifactu' ); ?>">
        ...
        <?php if ( ! empty( $vf_huella ) ) : ?>
            <div class="almc-vf-info-row">
                <strong><?php esc_html_e( 'Huella:', 'almc-verifactu' ); ?></strong>
        ...
        <?php if ( ! empty( $vf_last_error ) ) : ?>
            <div class="almc-vf-error-row">
                <strong><?php esc_html_e( 'Error:', 'almc-verifactu' ); ?></strong>
        ...
        <?php if ( ! empty( $vf_aeat_response ) ) : ?>
            <div class="almc-vf-aeat-response">
                <a href="#" class="almc-vf-toggle-response">
                    <?php esc_html_e( 'Ver respuesta AEAT', 'almc-verifactu' ); ?> &#9660;
                </a>
    ...
    <?php else : ?>
        ...
        <!-- No invoice yet -->
        <p><?php esc_html_e( 'Este pedido no tiene factura en VeriFactu.', 'almc-verifactu' ); ?></p>
